...
def visualise_gridworld(agent_environment: AgentEnvironment[tuple[int, int], int]):
    ...
    matrix = q_values_in_matrix(world, agent_environment.agent)
    q_mesh = ax2.pcolormesh(matrix.numpy(), vmin=-10, vmax=10)
    ax2.invert_yaxis()
    rewards = rewards_in_matrix(agent_environment)
    reward_mesh = ax3.pcolormesh(rewards.numpy(), vmin=-3, vmax=3)
    ax3.invert_yaxis()
    ...
    def update(frame):
        ...
        reward_mesh.set_array(rewards.numpy().flatten())
